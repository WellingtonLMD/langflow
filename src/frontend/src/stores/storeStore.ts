import { ENABLE_AUTOMATION_BUILDER_STORE } from "@/customization/feature-flags";
...
export const useStoreStore = create<StoreStoreType>((set) => ({
  hasStore: ENABLE_AUTOMATION_BUILDER_STORE,
  validApiKey: false,
  hasApiKey: false,
  loadingApiKey: true,
  checkHasStore: () => {
    checkHasStore().then((res) => {
      set({
        hasStore: ENABLE_AUTOMATION_BUILDER_STORE && (res?.enabled ?? false),
      });
    });
  },
